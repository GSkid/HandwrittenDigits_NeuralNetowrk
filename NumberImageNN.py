...
for t in range(num_iter):
    for i in range(len(img_in)):
        ...
        intermediate_guess = torch.matmul(weight1, hidden_node).clamp(min=0, max=1)
        ...
        # the second weight
        nn_guess = torch.matmul(weight2, intermediate_guess).clamp(min=0, max=1)
        ...
        error_vector = nn_guess - correct_result
        errors = error_vector.pow(2).sum()
